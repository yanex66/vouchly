...
def send_sms_alert(phone_number, message):
    ...
    """
    print(f"\n[SMS GATEWAY] To: {phone_number}")
    print(f"[SMS GATEWAY] Msg: {message}\n")
    return True
...
def cancel_order(request, order_id):
    """
    Allows a buyer to cancel an order that is still in PENDING status.
    """
...
def user_dashboard(request):
    ...
    if profile.user_type == 'ADVERTISER':
        ...
        revenue_data = Order.objects.filter(
            ...
            earnings=Sum(F('amount') - F('commission_earned'))
        )
...
def register(request):
    ...
    return render(request, 'core/register.html', {
        'form': form, 
        'is_forced_buyer': is_forced_buyer
    })
...
def edit_profile(request):
    """
    Unified Profile and User Update view.
    """
    profile = request.user.profile
    if request.method == 'POST':
        # Clean the whatsapp_number from the POST data before validation to prevent 'None' error
        ...
        if form.is_valid():
            ...
            if verified_name:
                profile.account_name = verified_name
            ...
            for field, errors in form.errors.items():
                for error in errors:
                    messages.error(request, f"{field.replace('_', ' ').upper()}: {error}")
    else:
...
def verify_bank_account(request):
    ...
    try:
        ...
        if res.get('status'):
            return JsonResponse({'status': True, 'account_name': res['data']['account_name']})
...
def checkout_desk(request):
    ...
    incoming_orders = Order.objects.filter(seller=request.user)\
        .exclude(status='PENDING')\
        .annotate(seller_earning=F('amount') - F('commission_earned'))\
        .order_by('-created_at')

    return render(request, 'core/checkout_desk.html', {
        'my_orders': my_orders,
